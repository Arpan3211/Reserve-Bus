...
export const posttripsController = async (req, res) => {
  ...
  // Create a new instance of the tripsModel with the timestamp
  const myData = new tripsModel({
    date: timestamp,
    from,
    to,
    busOwnerID,
  ...
  try {
    ...
    // Return success message with the saved data
    res.status(200).json({ data: savedData, message: "Booking successful!" });
  } catch (error) {
    // Return error message if there was an issue saving the document
